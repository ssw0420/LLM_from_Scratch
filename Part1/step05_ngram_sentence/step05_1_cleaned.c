/*
 * Step 5-1: 글자 단위 N-gram 문장 생성 (괄호/따옴표 제거 버전)
 *
 * data_cleaned.txt (괄호·따옴표 제거)로 학습하여 새 문장을 생성한다.
...
int main(void)
{
    ...
    /* ── 1. 데이터 로드 ── */
    int sent_count = 0;
    Sentence *sents = read_sentences("data_cleaned.txt", &sent_count);
    if (!sents) {
        fprintf(stderr, "data_cleaned.txt\xEB\xA5\xBC \xEC\x97\xB4 \xEC\x88\x98 "
    ...
    printf("=== Step 5-1: N-gram (cleaned) \xEB\xAC\xB8\xEC\x9E\xA5 \xEC\x83\x9D\xEC\x84\xB1 ===\n");
    ...
    const char *filenames[] = {
        "output_cleaned_n3.txt",
        "output_cleaned_n5.txt",
        "output_cleaned_n7.txt"
    };
